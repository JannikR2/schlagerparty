...
export async function beginTurn(game: Record<string, unknown>, seat: number) {
  ...
  const { data: updated, error: updateError } = await db.from("games").update({
    ...
    reveal_ends_at: null, clip_ends_at: new Date(now + clipMs).toISOString(), version: (game.version as number) + 1,
...
export async function finishByScore(gameId: string, version: number) {
  ...
  const { data, error } = await db.from("games").update({ phase: "finished", winner_ids: winnerIds, reveal_ends_at: null, clip_ends_at: null, version: version + 1 }).eq("id", gameId).eq("version", version).select().single();
  if (error) throw error;
  return data;
}
